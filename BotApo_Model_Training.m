%% Script to Train the Model to Identify the Bottom Aponeurosis 
clear
clc
...
labelIDs   = [0 255];
pxds = pixelLabelDatastore('D:\Sparsh\PURA\CleanedBotApoMasks\*.tif',classNames,labelIDs);
...
% trains the network
botApoNet = trainNetwork(pximdsTrain,lgraph,options);

% evaluating the trained network by segmenting a test image and displaying the segmentation result
I = imread('rawim_001.tif');
[pxdsResults,scores] = semanticseg(I,botApoNet);
B = labeloverlay(I,pxdsResults);
montage({I,B})
...
pxdsResults = semanticseg(imds,botApoNet,'MiniBatchSize',2,"WriteLocation",tempdir);
